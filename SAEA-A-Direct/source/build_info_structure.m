...
if params.max_pool_size < params.init_sample_size
    ...
    info.pool.behavior_vector = zeros(info.params.max_pool_size,size(X,2)); % Behavior correction
    info.pool.CR =  0.8.*ones(info.params.max_pool_size,1);     % Crossover parameter
    
else
    info.pool.X = X;
    info.pool.y = y;
    info.pool.age = ones(size(X,1),1);
    info.pool.behavior_vector = zeros(size(X,1),size(X,2)); % Behavior correction
    info.pool.CR =  0.8.*ones(size(X,1),1);     % Crossover parameter
end

% To prevent a poorly metamodel sample
info.pool.poorly_quality = 0;
info.pool.Xsample = X(1:params.sample_size,:);
info.pool.Ysample = y(1:params.sample_size);
