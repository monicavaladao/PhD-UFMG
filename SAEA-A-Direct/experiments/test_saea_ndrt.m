...
%aux = load_analytic_problem('hybrid_composition',50);
%aux = load_analytic_problem('rosenbrock_func', 100);
%aux = load_analytic_problem('rosenbrock_func', 20);
%aux = load_analytic_problem('sphere', 20);
aux = load_analytic_problem('ackley', 20);
...
npop = 50 + floor(n/10); % Está bom esse valor de npop
%npop = 80 + floor(n/10); % Está bom esse valor de npop
%npop = 100 + floor(n/10);
...
[best_x, best_y, info] = surrogate_tasea_slpso_ndrt(fobj, X, y, lb, ub, max_eval)
