...
strategy = 'tasea_slpso_self_update';
...
%metamodels(1).name = 'rbf-gaussian';
%metamodels(1).params = {'Metamodel', 'RBF_SRGTSToolbox', 'RBF', 'Gaussian', 'Verbose', false};
%metamodels(2).name = 'simple-kriging';
%metamodels(2).params = {'Metamodel', 'SimpleKriging_ooDACE', 'Verbose', false};
%metamodels(3).name = 'ordinary-kriging';
%metamodels(3).params = {'Metamodel', 'OrdinaryKriging_ooDACE', 'Verbose', false};


metamodels(1).name = 'rbf-multiquadric';
metamodels(1).params = {'Metamodel', 'RBF_SRGTSToolbox', 'RBF', 'Multiquadric', 'Verbose', false};
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% Problems to solve
nvars = [10, 20, 30, 50, 100, 200];
npop = [50 + floor(nvars(1)/10), 50 + floor(nvars(2)/10), 50 + floor(nvars(3)/10),...
         50 + floor(nvars(4)/10), 50 + floor(nvars(5)/10), 80 + floor(nvars(6)/10)];
         
 neval = [2000, 2000, 2000, 2000, 2000, 2000];
