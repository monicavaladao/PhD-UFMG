...
dir_output = './results_noSELFUPDATE/analytic';
...
strategy = 'tasea_slpso_noSELFUPDATE';
...
metamodels(1).name = 'simple-kriging';
metamodels(1).params = {'Metamodel', 'SimpleKriging_ooDACE', 'Verbose', false};
...
nthreads = 1;%4;                % num. of threads to use
c = parcluster();            % cluster
p = parpool(c, nthreads);    % pool
for i = 1:length(entries)
    f(i) = parfeval(p, @launch_noSELFUPDATE, 0, entries(i).problem, ...
...
fid = fopen(strcat(dir_output, '/results_noSELFUPDATE.csv'), 'w+');
