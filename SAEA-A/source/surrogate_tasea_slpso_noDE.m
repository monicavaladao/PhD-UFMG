function [best_x, best_y, info] = surrogate_tasea_slpso_noDE(fobj, X, y, lb, ub, max_eval, varargin)
% SURROGATE_TASEA_SLPSO_NODE: Surrogate Assisted Evolutionary Algorithm  build over
% ooDACE toolbox and SRGTS toolbox.
%--------------------------------------------------------------------------
% Remark:
% This function implement the "surrogate_tasea_slpso" without DE operators
% in order to compare with the surrogate_tasea_slpso algorithm.
%--------------------------------------------------------------------------
% Input:
...
while eval_counter < params.max_eval
    ...
    % Create offspring solutions using SL_PSO operator
    [P,P_V,P_CR] = create_offsprings_slpso(pop_X, pop_y, pop_V, pop_CR, lb, ub, bestever);
            
    [x_b, y_b, pred_b, v_b, cr_b, imp_b, info] = prescreening_solution(fobj, P, P_V, P_CR, best_x,...
