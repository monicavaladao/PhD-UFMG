...
%npop = 80 + floor(nvars/10);
ssize = 4*npop;%500;%200;%100;%95; % Size of initial sample
X = lhsdesign(ssize, n);
X = repmat(lb, ssize, 1) + repmat(ub - lb, ssize, 1) .* X;
y = feval_all(fobj, X);

% Perform the TASEA algorithm 
[best_x, best_y, info] = surrogate_gsga(fobj, X, y, lb, ub, max_eval)
