...
%
% Adaptated to use the DIRECT algorithm to estimate theta
% Monica A. C. Valadao
% June 23, 2020
%
...
if  nargin > 5
    ...
    objfunc_direct = @(theta)(objfunc(theta, par));
    Problem.f = objfunc_direct;
    bounds_direct = [lob, upb];
    %opts_direct = [1e-3,20,10,100,1];
    opts_direct = struct();
    opts_direct.ep        =  1e-4;
    opts_direct.maxevals  = 20;
    opts_direct.maxits    = 10;
    opts_direct.maxdeep   = 100;
    opts_direct.showits   = 0;
    [f,theta,~] = Direct(Problem,bounds_direct,opts_direct);
    theta = theta';
